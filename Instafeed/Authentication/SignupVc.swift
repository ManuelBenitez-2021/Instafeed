...
class SignupVc: UIViewController {
   ...
   // @IBOutlet weak var countrypick: CountryPicker!
    ...
    fileprivate func checkFields(){
        ...
        }else if mobile.text?.count != 10 {
         CommonFuncs.AlertWithOK(msg: "Mobile number is required 10 characters", vc: self)
    ...
    func onDetectFirstName(name: String) -> String {
        var firstName = ""
        var cnt = 0
        for chr in name {
            cnt = cnt + 1
            if cnt == 1 {
                firstName = firstName + String(chr)
                continue
            }
            if chr.isUppercase {
                break
    ...
    fileprivate func signUpuser(){
        ...
        let firstName = self.onDetectFirstName(name: fullname.text!)
        
        let parameters = ["first_name":firstName, "email":email.text!, "mobile":mobile.text!, "device_id":"not now", "password":password.text!, "refer_code":referCode.text!]
...
//                                    if let imageURL = ((result?["picture"] as? [String: Any])?["data"] as? [String: Any])?["url"] as? String {
                                    if let imageURL = (((result! as AnyObject).value(forKey: "picture") as AnyObject).value(forKey: "data") as AnyObject).value(forKey: "url") as? String{
                                            self?.fb_userData["profile_pic"] = imageURL
                                        }
    ...
    fileprivate func socialLogin_APIcall(provider:String, DataDict:[String:Any]){
        ...
        let img   = DataDict["profile_pic"] as! String
        
        
        let params = ["social_id":token, "provider_id":provider, "first_name":fname, "last_name":lname, "email":email, "phone":"", "password":"", "image":img]
        print(params)
...
extension SignupVc : GIDSignInDelegate,GIDSignInUIDelegate{
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!,
        ...
        if (error == nil) {
            ...
            let img = user.profile.imageURL(withDimension: 120)?.absoluteString
            // ...
            print(fullName!)
            print(givenName!)
            print(familyName!)
            ...
            Gmail_userData["profile_pic"] = img!
